...
class ArchSummaryMessage(BaseMessage):
    ...
    def __str__(self):
        return f"[{self.role.upper()}@ARCHSTEP] {self.content}"
...
class FullInputMemory(BaseChatMessageHistory):
        ...
        if len(self.messages) <= self.max_prompt_history:
            ...
            for msg in selected_msgs:
                ...
                if prev_step is not None and step is not None and step - prev_step > 1:
                    ...
                    final_msgs.append(ArchSummaryMessage(content=skip_text, meta={"step": ' '}))
    ...
    def save_context(self, inputs: Dict, outputs: Dict):
        ...
        if step == 1:
            self.messages.append(ArchSummaryMessage(content=f"Generated Architecture:\n{str(outputs.get('result',''))}", meta={"step": step}))
        elif step > 1:
            self.messages.append(ArchSummaryMessage(content=f"The feedback of the STEP {step-1} is:\n{inputs.get('feedback','')}\nGiven the feedback of STEP {step-1}, the diff between the updated architecture and the previous architecture is:\n{outputs.get('arch_diff','')}\n", meta={"step": step, "feedback": inputs.get("feedback","")}))
