...
* Share an instance of a `BPlusTree` pointing to the same file across multiple processes
...
If the tree doesn't get closed properly (power outage, process killed...) the WAL file is merged the next time the tree is opened.
...
Some advice to efficiently use the tree:
